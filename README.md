...
### 👨‍👩‍👧‍👦 목표 서비스 시나리오 (NAS 가족 공유형)
- **NAS에서 직접 구동되는 웹서비스**로, 사진을 안전하게 업로드하고 가족에게만 공유하는 용도
- **Docker 이미지 배포**를 기본으로 하며, **스토리지 경로는 `.env`로 설정**
- **권한 구조**는 마스터/업로더/뷰어로 단순화
- **공유 범위는 앨범 단위**로 관리
- **가입 방식은 초대 링크 기반**으로 제한
- **댓글 권한**은 작성자 및 게시글 업로드자(앨범 업로드자 포함)에게만 허용
...
## 🔐 권한/공유 정책 (NAS 가족 공유형 목표)

| 역할 | 권한 |
|------|------|
| **Master** | 계정/권한 관리, 앨범 공유 정책 설정 |
| **Uploader** | 앨범 생성 및 이미지 업로드 |
| **Viewer** | 공유된 앨범 열람 및 댓글 작성 |

**규칙**
- 공유 범위는 **앨범 단위**
- **댓글 삭제/수정**은 댓글 작성자 또는 해당 앨범 업로드자만 가능
- **가입은 초대 링크 기반**으로만 허용
...
### 1) .env 구성
NAS에 마운트된 디렉터리를 `IMAGE_STORAGE_PATH`로 지정합니다. 컨테이너 내부 경로는 필요 시 `IMAGE_STORAGE_CONTAINER_PATH`로 변경할 수 있습니다.
...
```env
IMAGE_STORAGE_PATH=/mnt/nas/imageviewer/images
IMAGE_STORAGE_CONTAINER_PATH=/app/storage/images
```

### 2) docker-compose 실행
`.env` 값을 사용해 **NAS 경로를 컨테이너 내부로 마운트**합니다.
...
### 3) 이미지 저장 경로
컨테이너 내부에서는 아래 경로로 저장됩니다. 필요 시 `.env`에서 변경 가능합니다.
...
### 4) NAS 권한/경로 체크
NAS 경로는 **컨테이너가 쓰기 가능한 권한**이 있어야 하며, 미리 디렉터리를 생성해 두는 것을 권장합니다.
